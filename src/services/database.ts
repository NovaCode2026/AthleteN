...
export const TABLES = {
  profile: "profiles", academies: "academies", academyMemberships: "academy_memberships", training: "training_sessions", trainingPlans: "training_plans", attendance: "attendance_records", tournaments: "tournaments", matches: "matches", medals: "medals", certificates: "certificates", documents: "documents", weights: "weight_logs", calendar: "calendar_events", notifications: "notifications", checklist: "competition_checklists", injuries: "injuries", goals: "goals", feedback: "feedback_items", verifications: "student_verifications", roadmap: "roadmap_items", roadmapVotes: "roadmap_votes", tournamentScans: "tournament_scans", referrals: "referrals", badges: "athlete_badges", aiUsage: "ai_usage_events", subscriptions: "subscriptions", subscriptionUsage: "subscription_usage", paymentEvents: "payment_events", supportTickets: "support_tickets", announcements: "announcements", featureFlags: "feature_flags", auditLogs: "audit_logs"
} as const;
...
function toDatabaseError(resource: Resource, action: string, error: unknown) {
  ...
  const safeDetail=import.meta.env.DEV?` ${detail}`:action==="save"?" Please check the fields and try again.":" Please try again later.";
  return new Error(`Unable to ${action} ${resource}.${safeDetail}`);
}

function normalizeInsertValues(resource: Resource, values: Record<string, unknown>) {
  if (resource !== "documents") return values;
  // The documents table stores only title, type, file path and expiry metadata.
  // Older UI fields such as issued_at/notes must not be sent to PostgREST.
...
export async function listRows<T>(resource: Resource, userId?: string, options: { order?: string; ascending?: boolean; publicRows?: boolean } = {}) {
  const table=TABLES[resource]; let query=requireSupabase().from(table).select("*");
  if(!options.publicRows&&userId)query=query.eq("user_id",userId);
  if(options.order)query=query.order(options.order,{ascending:options.ascending??false});
  const {data,error}=await query; if(error)throw toDatabaseError(resource,"load",error); return (data??[]) as T[];
}
export async function upsertRow<T extends Record<string,unknown>>(resource: Resource, values: T, options:{onConflict?:string}={}) {
  const table=TABLES[resource]; const payload=normalizeInsertValues(resource,values);
  ...
  const {data,error}=await query.select().single(); if(error)throw toDatabaseError(resource,"save",error); return data;
}
export async function insertRow<T extends Record<string,unknown>>(resource: Resource, values:T) {
  const table=TABLES[resource]; const payload=normalizeInsertValues(resource,values);
  const {data,error}=await requireSupabase().from(table).insert(payload).select().single(); if(error)throw toDatabaseError(resource,"save",error); return data;
}
export async function updateRow<T extends Record<string,unknown>>(resource: Resource,id:string,values:T,userId?:string) {
  const table=TABLES[resource]; let query=requireSupabase().from(table).update(values).eq("id",id); if(userId)query=query.eq("user_id",userId);
  const {data,error}=await query.select().single(); if(error)throw toDatabaseError(resource,"save",error); return data;
}
...
export async function insertManyRows<T extends Record<string,unknown>>(resource: Resource,values:T[]) { const table=TABLES[resource]; const payloads=values.map(value=>normalizeInsertValues(resource,value)); const {data,error}=await requireSupabase().from(table).insert(payloads).select(); if(error)throw toDatabaseError(resource,"save",error); return data; }
...
export async function uploadPrivateFile(bucket:string,path:string,file:File) {
  ...
  const {data,error}=await requireSupabase().storage.from(bucket).upload(path,file,{upsert:true,cacheControl:"3600",contentType:file.type||"application/octet-stream"});
  if(error)throw new Error(import.meta.env.DEV?error.message:"Unable to upload the file securely. Please try again."); return data.path;
}
