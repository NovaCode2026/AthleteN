...
function AppShell() {
  ...
  async function save(resource: Resource, values: Record<string, string | number | boolean>, options: { rethrow?: boolean } = {}) {
    try {
      ...
      if (resource === "documents" && (!values.title || !values.document_type || !values.file_path)) throw new Error("Document name, type, and file are required.");
      if (resource === "verifications" && (!values.document_type || !values.file_path)) throw new Error("Proof type and proof file are required.");
  ...
  return <div className="app-shell">
    ...
    <Toast toast={toast} />
    {form && <RecordModal form={form} setForm={setForm} save={save} profile={data.profile} userId={auth.user?.id} setToast={setToast} />}
  </div>;
}
...
function RecordModal({ form, setForm, save, profile, userId, setToast }: RecordModalProps) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (form.resource === "verifications" || form.resource === "documents") {
        if (!file) throw new Error(form.resource === "verifications" ? "Choose a proof file before submitting." : "Choose a document file before saving.");
        if (!userId) throw new Error("Please sign in again before uploading.");
        const extension = file.name.includes(".") ? file.name.split(".").pop()?.toLowerCase() : undefined;
        const allowedExtensions = ["pdf", "png", "jpg", "jpeg", "webp"];
        if (!extension || !allowedExtensions.includes(extension)) {
          throw new Error("Choose a PDF, PNG, JPG, JPEG, or WEBP file.");
        }
        const safeName = `${crypto.randomUUID()}.${extension}`;
        const bucket = form.resource === "verifications" ? "verification-proofs" : "documents";
        uploadedPath = await uploadPrivateFile(bucket, `${userId}/${safeName}`, file);
        payload.file_path = uploadedPath;
      }
      if (form.resource === "verifications") {
        if (!payload.document_type) throw new Error("Choose the proof type.");
        payload.status = "pending";
      }
      await save(form.resource, payload, { rethrow: true });
    } catch (error) {
      if (uploadedPath) {
        try {
          await deletePrivateFile(form.resource === "verifications" ? "verification-proofs" : "documents", uploadedPath);
        } catch {
