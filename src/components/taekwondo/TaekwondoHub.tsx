...
const POOMSAE_FORMS = [
  "Taegeuk 1 Jang", "Taegeuk 2 Jang", "Taegeuk 3 Jang", "Taegeuk 4 Jang", "Taegeuk 5 Jang", "Taegeuk 6 Jang", "Taegeuk 7 Jang", "Taegeuk 8 Jang",
  "Koryo", "Keumgang", "Taebaek", "Pyongwon", "Sipjin", "Jitae", "Cheonkwon", "Hansu", "Ilyeo",
];
const POOMSAE_CATEGORIES = ["Individual", "Pair", "Team", "Freestyle", "Other"];
...
export default function TaekwondoHub({ profile, userId, kyorugiBouts: initialKyorugi = [], poomsaePerformances: initialPoomsae = [], taekwondoTraining: initialTraining = [], setToast, refresh }: Props) {
  ...
  async function saveTraining() {
    ...
    if (discipline === "poomsae" && !form.poomsae) { setError("Choose the Poomsae performed."); return; }
    setBusy(true); setError("");
    try {
      const payload = {
        user_id: athleteId,
        ...
        is_official: false,
        ...(discipline === "kyorugi"
          ? { rounds: form.rounds ? Number(form.rounds) : null, repetitions: null, poomsae_name: null, category: null }
          : { repetitions: form.repetitions ? Number(form.repetitions) : null, rounds: null, poomsae_name: form.poomsae, category: form.category || null }),
      };
  ...
  async function saveCompetition() {
    ...
    try {
      if (discipline === "kyorugi") {
        ...
        if (!form.poomsae) { setError("Choose the Poomsae performed."); return; }
        const score = form.score === "" || form.score === undefined ? null : Number(form.score);
        const placing = form.placing === "" || form.placing === undefined ? null : Number(form.placing);
        if (score !== null && (!Number.isFinite(score) || score < 0 || score > 10)) { setError("Poomsae score must be between 0 and 10."); return; }
        if (placing !== null && (!Number.isInteger(placing) || placing < 1)) { setError("Placing must be a positive whole number."); return; }
        await insertRow("poomsaePerformances", { user_id: athleteId, event_name: form.event, event_date: form.date, poomsae_name: form.poomsae, category: form.category || null, score, placing, result: form.result || null, notes: form.notes || null, is_official: true });
      }
...
function Select({ label, value, onChange, options, required }: { label: string; value: string; onChange: (event: React.ChangeEvent<HTMLSelectElement>) => void; options: string[][]; required?: boolean }) { return <label className="field"><span>{label}</span><select required={required} value={value} onChange={onChange}><option value="">Select</option>{options.map(([v, t]) => <option key={v} value={v}>{t}</option>)}</select></label>; }
