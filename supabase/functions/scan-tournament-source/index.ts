...
function meta(html:string,key:string){
  const re=/<meta[^>]+(?:property|name)=["']([^"']+)["'][^>]+content=["']([^"']*)["'][^>]*>/gi;let m;
  while((m=re.exec(html)))if(String(m[1]).toLowerCase()===key.toLowerCase())return clean(m[2]);
  return "";
}
function pageTitle(html:string){const m=html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);return clean(m&&m[1]);}
function bodyText(html:string){return clean(html.replace(/<script[\s\S]*?<\/script>/gi," ").replace(/<style[\s\S]*?<\/style>/gi," ").replace(/<[^>]+>/g," "));}
...
function findDate(text:string){
  const m=text.match(/\b(?:[0-3]?\d(?:st|nd|rd|th)?\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)(?:\s+\d{4})?|(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+[0-3]?\d(?:st|nd|rd|th)?(?:,?\s+\d{4})?)\b/i);
  return m?m[0]:"";
}
function findVenue(text:string){
  const m=text.match(/\b(?:Ranikhet|Delhi|Noida|Kanpur|Lucknow|Jaipur|Mumbai|Pune|Chandigarh|Bengaluru|Hyderabad|Kolkata|Gurugram|Gurgaon|Dehradun|Uttarakhand|Uttar Pradesh|Rajasthan|Maharashtra|Punjab|Haryana)\b(?:,\s*[A-Z][A-Za-z .'-]+)?/i);
  return m?clean(m[0]):"";
}
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  let res:Response;
  try{res=await fetch(sourceUrl,{redirect:"follow",headers:{"User-Agent":"AthleteN-Scanner/1.0"}})}
  catch(e){await admin.from("tournament_scans").update({status:"failed",detected_changes:"Unable to fetch source: "+String(e).slice(0,300),last_checked_at:new Date().toISOString()}).eq("id",scanId).eq("user_id",user.id);return new Response(JSON.stringify({status:"failed"}),{headers:{...cors,"Content-Type":"application/json"}})}
  ...
  const name=meta(html,"og:title")||pageTitle(html);
  const desc=meta(html,"og:description")||meta(html,"description");
  const text=bodyText(html);
  const combined=clean(name+" "+desc+" "+text);
  const date=findDate(combined);
  const venue=findVenue(combined);
  const hashValue=await hash(html);
  const changed=!!found.data.source_hash&&found.data.source_hash!==hashValue;
  const update={
    tournament_name:name||null,
    tournament_date:date||null,
    venue:venue||null,
    notices:desc||"Source fetched successfully. Review the official page for full details.",
    pdfs:getPdfs(html,sourceUrl),
    schedules_results:getLinks(html,sourceUrl).join("\n")||null,
    source_hash:hashValue,
    detected_changes:changed?"Source content changed since the previous scan.":"Baseline scan completed. No previous source version existed.",
    status:"checked",
