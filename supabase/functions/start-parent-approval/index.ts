...
export default {
  fetch: withSupabase({ auth: 'user' }, async (req, ctx) => {
    ...
    try {
      ...
      const resendKey = Deno.env.get('RESEND_API_KEY'); const from = Deno.env.get('PARENT_APPROVAL_FROM'); const siteUrl = (Deno.env.get('ATHLETEN_PUBLIC_URL') || Deno.env.get('SITE_URL') || '').replace(/\/$/, '')
      if (!resendKey || !from || !siteUrl) return json({ error: 'Parent approval email service is not configured yet. Set RESEND_API_KEY, PARENT_APPROVAL_FROM, and ATHLETEN_PUBLIC_URL.' }, 503)
