...
create policy "feature controls athlete read" on public.athlete_feature_controls for select to authenticated using (athlete_user_id = (select auth.uid()) or public.can_manage_athlete_feature(athlete_user_id));
create policy "feature controls coach manage" on public.athlete_feature_controls for all to authenticated using (public.can_manage_athlete_feature(athlete_user_id)) with check (public.can_manage_athlete_feature(athlete_user_id));

drop policy if exists "nutrition own rows" on public.nutrition_logs;
create policy "nutrition own rows" on public.nutrition_logs for all to authenticated using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()) and exists (select 1 from public.athlete_feature_controls where athlete_user_id = (select auth.uid()) and feature_key = 'nutrition' and enabled = true));
drop policy if exists "hydration own rows" on public.hydration_logs;
create policy "hydration own rows" on public.hydration_logs for all to authenticated using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()) and exists (select 1 from public.athlete_feature_controls where athlete_user_id = (select auth.uid()) and feature_key = 'nutrition' and enabled = true));

drop trigger if exists set_athlete_feature_controls_updated_at on public.athlete_feature_controls;
create trigger set_athlete_feature_controls_updated_at before update on public.athlete_feature_controls for each row execute function public.set_updated_at();
