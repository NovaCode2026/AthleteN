...
function IconTile({ icon, tone }: { icon: string; tone: 'blue' | 'green' | 'red' | 'purple' | 'slate' }) {
  ...
  return <View style={{ width: 58, height: 58, borderRadius: 18, backgroundColor: bg, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color, fontSize: 27, fontWeight: '900' }}>{icon}</Text></View>;
}
...
function ActionCard({ icon, tone, title, subtitle, onPress }: { icon: string; tone: 'blue' | 'green' | 'red' | 'purple'; title: string; subtitle: string; onPress: () => void }) {
  return <Pressable onPress={onPress} style={({ pressed }) => [{ flex: 1, minWidth: '46%' }, pressed && { opacity: 0.78 }]}>
    <Card>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 13 }}>
        <IconTile icon={icon} tone={tone} />
        <View style={{ flex: 1 }}>
          <Text style={{ color: c.text, fontSize: 15, fontWeight: '900' }}>{title}</Text>
          <Text style={{ color: c.muted, fontSize: 11, lineHeight: 16, marginTop: 4 }}>{subtitle}</Text>
        </View>
      ...
      <View style={{ gap: 22 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 4 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
            <View style={{ width: 54, height: 54, borderRadius: 16, backgroundColor: '#02060c', borderWidth: 1, borderColor: c.accentDeep, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: '#fff', fontSize: 25, fontWeight: '900' }}>A</Text>
              <Text style={{ color: c.accentBright, fontSize: 13, fontWeight: '900', position: 'absolute', right: 8, bottom: 8 }}>N</Text>
            </View>
            <View>
              <Text style={{ color: c.text, fontSize: 17, fontWeight: '900', letterSpacing: 4 }}>ATHLETEN</Text>
              <Text style={{ color: c.muted, fontSize: 7, letterSpacing: 2, fontWeight: '900' }}>ATHLETE PERFORMANCE PLATFORM</Text>
            </View>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            <Pressable onPress={() => router.push('/messages')} style={{ width: 42, height: 42, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: c.text, fontSize: 27 }}>♧</Text>
              <View style={{ position: 'absolute', right: 4, top: 6, width: 8, height: 8, borderRadius: 4, backgroundColor: c.accentBright }} />
            </Pressable>
            <Pressable onPress={() => router.push('/coach-profile')} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              {avatar ? <Image source={{ uri: avatar }} style={{ width: 48, height: 48, borderRadius: 24 }} contentFit="cover" /> :
                <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: '#17335e', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#6faaff', fontSize: 20, fontWeight: '900' }}>{initial}</Text></View>}
              <View><Text style={{ color: c.text, fontSize: 14, fontWeight: '900' }}>{profile?.full_name || 'Coach'}</Text><Text style={{ color: c.muted, fontSize: 11 }}>View profile  ›</Text></View>
            </Pressable>
          </View>
        </View>
        ...
        <View style={{ gap: 7 }}>
          ...
          <Text style={{ color: c.text, fontSize: 36, fontWeight: '900' }}>{firstName}<Text style={{ color: c.accentBright }}>.</Text></Text>
          <Text style={{ color: c.muted, fontSize: 16, fontWeight: '700' }}>Train. Compete. Develop. <Text style={{ color: c.textSecondary }}>All in one place.</Text></Text>
        </View>
        ...
        <Card>
          <View style={{ flexDirection: 'row' }}>
            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View><Text style={{ color: c.muted, fontSize: 12 }}>Your Athletes</Text><Text style={{ color: c.text, fontSize: 25, fontWeight: '900', marginTop: 3 }}>{athletes.length}</Text><Text style={{ color: c.muted, fontSize: 11 }}>Active athletes</Text></View>
            </View>
            <View style={{ width: 1, backgroundColor: c.borderStrong, marginVertical: 5 }} />
            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 12, paddingLeft: 18 }}>
              <IconTile icon="▣" tone="blue" />
              <View><Text style={{ color: c.muted, fontSize: 12 }}>Upcoming Events</Text><Text style={{ color: c.text, fontSize: 25, fontWeight: '900', marginTop: 3 }}>{events}</Text><Text style={{ color: c.muted, fontSize: 11 }}>Next 30 days</Text></View>
            </View>
          </View>
        </Card>
        ...
        <View style={{ gap: 10 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Pressable onPress={() => router.push('/coach-more')}><Text style={{ color: c.accentBright, fontWeight: '900' }}>View all  ›</Text></Pressable>
          </View>
          <Card>
        ...
        <Pressable onPress={() => router.push('/scanner')} style={{ marginBottom: 8 }}>
          <Card accent><Text style={{ color: c.accentBright, fontSize: 10, fontWeight: '900', letterSpacing: 1 }}>COACH TOOL</Text><Text style={{ color: c.text, fontSize: 15, fontWeight: '900', marginTop: 4 }}>Tournament Scanner</Text><Text style={{ color: c.muted, fontSize: 11, marginTop: 2 }}>Track tournament sources and bring useful event intelligence into your workspace.</Text></Card>
        </Pressable>
      </View>
    </Screen>
  );
