...
export default async function handler(request) {
  ...
  try {
    ...
    const { error: stateError } = await supabase.from("instagram_oauth_states").insert({ state, user_id: data.user.id, expires_at: new Date(Date.now() + 10 * 60 * 1000).toISOString() });
    ...
    authorizeUrl.searchParams.set("client_id", appId); authorizeUrl.searchParams.set("redirect_uri", redirectUri); authorizeUrl.searchParams.set("response_type", "code"); authorizeUrl.searchParams.set("scope", scopes); authorizeUrl.searchParams.set("state", state);
    return json({ authorizeUrl: authorizeUrl.toString() });
  } catch (error) { console.error("instagram-connect", error); return json({ error: "Unable to start Instagram authorization." }, 503); }
}
