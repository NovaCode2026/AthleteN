    ...
    try {
      ...
      const reportingDateCandidates = normalizedEvidenceLines
        ...
        .flatMap((line) => {
          ...
          for (const pattern of patterns) {
            ...
            if (match?.[1] && match?.[2]) values.push(match[1] + " " + match[2] + (match[3] ? " " + match[3] : ""));
          }
          return values;
        })
        .filter(Boolean);
      ...
      if (reportingDateCandidates.length) {
        const plausible = reportingDateCandidates
          .map((value) => ({ value, iso: toDatabaseDate(value) }))
          .filter((item) => {
            const eventIso = toDatabaseDate(poster.event_date_text || "");
...
async function scanPublicSource(sourceUrl) {
  ...
  const reportingDateForStructuredOutput = (() => {
    ...
    const eventIso = toDatabaseDate(resolvedEventDateText);
    if (eventIso) {
      const reportTime = new Date(reportIso + "T00:00:00Z").getTime();
      const eventTime = new Date(eventIso + "T00:00:00Z").getTime();
    ...
    if (eventMatch && reportMatch) {
      ...
      const reportMonth = months[reportMatch[2].toLowerCase()];
      const eventStart = Number(eventMatch[1]);
      const reportDay = Number(reportMatch[1]);
      if (eventMonth && reportMonth && eventMonth === reportMonth && Number.isFinite(eventStart) && Number.isFinite(reportDay) && reportDay > eventStart) return "";
    }
    return raw;
  })();
