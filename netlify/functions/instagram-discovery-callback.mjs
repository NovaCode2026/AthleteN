...
function redirect(params) {
  const url = new URL("https://athleten.netlify.app/scanner");
...
export default async function handler(request) {
  ...
  try {
    ...
    const { error: saveError } = await supabase.from("instagram_discovery_connections").upsert({ user_id: oauthState.user_id, facebook_user_id: mePayload.id, instagram_user_id: page.instagram_business_account.id, instagram_username: page.instagram_business_account.username || null, access_token: accessToken, token_expires_at: new Date(Date.now() + expiresIn * 1000).toISOString(), connected_at: new Date().toISOString(), updated_at: new Date().toISOString(), status: "active" }, { onConflict: "user_id" });
